...
def enabled_lab_supervised_rows() -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    """
    state_path = ROOT / "datasets" / "catalog_v008.json"
    try:
    ...
    user = ROOT / "datasets" / "raw" / "user_entries_v008.jsonl"
    if user.is_file():
...
def tokenize() -> dict[str, Any]:
    ...
    report = {
        "format": "jarvis-v17-tokenization-report-v1",
        "dataset_version": VERSION,
        ...
        **lab_report,
        ...
        "note": "v17 uses dataset_v008 and merges enabled LAB supervised rows without duplicating built-in corpus files.",
        "pretrained_source": None,
    }
    metrics = ROOT / "models" / "tokenizer_metrics_v008.json"
