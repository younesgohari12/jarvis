...
def tokenize() -> dict[str, Any]:
    ...
    train = rows(ROOT / "datasets" / "splits" / "dataset_v004_train.jsonl")
    validation = rows(ROOT / "datasets" / "splits" / "dataset_v004_validation.jsonl")
    output_rows = []
    token_counts=[]
    bytes_total=0
    round_trip=0
    ...
    out = ROOT / "datasets" / "tokenized" / "dataset_v004_train.jsonl"
    ...
    for text in validation_texts:
        ...
        total_texts += 1
    report={
        "format":"jarvis-v12-tokenization-report-v1",
    ...
    (ROOT/"models"/"tokenizer_metrics_v004.json").write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    return report
