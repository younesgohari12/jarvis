...
class FollowupIntentModel:
    """Project-trained classifier for conversational continuation intent."""

    FORMAT = "jarvis-dialogue-followup-v1"
    ...
    def predict(self, text: str) -> tuple[str, float, float]:
        if not self.ready:
            return "", 0.0, 0.0
        vector = PersianFluencyEngine.weighted_vector(
            text, self._idf, allowed=set(self._idf)
        )
        ranked = sorted(
            (
