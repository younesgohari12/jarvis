"""v22.4.2 NEW blind diagnostic runner (spec §26).

The set is frozen (SHA256 recorded before execution) — this script never
modifies it. Methodology mirrors the established runner: tests.helpers
TemporaryRuntime, tools disabled, rt.agent.respond per case, numeric grading.

Grading:
  num      last number in the reply ~= expected
  abstain  pass iff no numeric answer is fabricated

Output: reports/v22_4_2/diagnostic.json
"""
...
BASE = '/home/z/my-project/jarvis_work/pkg/Jarvis_v0.11.0'
CASES = BASE + '/benchmarks/v22_4_2_diagnostic_set.jsonl'
OUT_DIR = BASE + '/reports/v22_4_2'
    ...
    with TemporaryRuntime() as rt:
        ...
        for i, row in enumerate(rows):
            ...
            def blocked(tool, *args, **kwargs):
                ...
                raise AssertionError('tools disabled for diagnostic')
        ...
        summary = {
            'set': 'V22_4_2_NEW_DIAGNOSTIC (freshly authored, never tuned)',
            'cases_file': 'benchmarks/v22_4_2_diagnostic_set.jsonl',
            'sha256': sha,
            'total': len(results),
            ...
            'note': ('this diagnostic is NOT the published frozen benchmark; '
                     'scores are reported separately and never averaged '
                     'into the frozen blind result (spec §38)'),
        }
    import os
