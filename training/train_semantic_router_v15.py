...
MODEL = ROOT / "models" / "semantic_router_v15.npz"
REPORT = ROOT / "training" / "semantic_router_v15_metrics.json"
FEATURE_SIZE = 262_144
SEED = 15092026
LABELS = SemanticIntentRouterV15.DEFAULT_LABELS


def rows(split: str) -> list[dict]:
    path = DATA / f"dataset_v007_{split}.jsonl"
    out = []
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            item = json.loads(line)
            if item.get("origin") == "jarvis-routing-v15":
                out.append(item)
    return out
...
def vectorize(items: Iterable[dict]) -> tuple[csr_matrix, np.ndarray]:
    ...
    for item in items:
        idx, val = SemanticIntentRouterV15.sparse_features(item["input"], FEATURE_SIZE)
...
def main() -> None:
    ...
    metrics = {
        "format": "jarvis-semantic-router-v15-training-report",
    ...
    np.savez_compressed(
        MODEL,
        format=np.asarray(SemanticIntentRouterV15.FORMAT),
