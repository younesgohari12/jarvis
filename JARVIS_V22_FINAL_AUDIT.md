...
## ۹) پیوست v22.2 — راستی‌آزمایی مستقل پیش از ارسال نهایی (2026-09-19)

پیش از ارسال نهایی، کل بستهٔ v22.1 از روی آرتیئفکت منتشرشدهٔ گیت‌هاب دانلود و مستقلاً بازبینی شد. نتیجه‌ها:

### ۹-۱) یافته: آرتیئفکت رسمی مغایر (رفع شد)
فایل `reports/v22/v22_fresh_blind_v221_official.json` در بستهٔ v22.1 خروجی یک رانر قدیمی (v20) بود که الگوهای پاسخ v22 را نمی‌شناخت: پاسخ‌های درست (مثلاً «1787» برای 1787) را `passed=False` ثبت کرده و جمعبندی `8.4%` نوشته بود — در تضاد آشکار با گزارش صادقانهٔ 83.58%. این فایل با یک اجرای رسمی تازه از `benchmarks/run_v22_fresh_blind.py` جایگزین شد.

### ۹-۲) بازتولید‌پذیری گزارش صادقانه
- اجرای مستقل پیش از تغییرات کد: **855/1023 — honest 83.58، answerable-only 82.09** (دقیقاً برابر گزارش).
- اجرای مستقل پس از تغییرات کد v22.2: **855/1023 — همان اعداد**، همهٔ خانواده‌ها یک‌به‌یک برابر (هیچ سطر بنچمارک رفتارش عوض نشد).
- مقایسهٔ خانواده‌به‌خانواده بین ران و گزارش منتشرشده: 14/14 مطابقت کامل.

### ۹-۳) باگ واقعی کشف‌شده و رفع‌شده: «It is 23:00 now …» → 25
جملهٔ «It is 23:00 now. The meeting lasts 2 hours. When does it end?» به‌اشتباه به‌صورت زنجیرهٔ جمع 23+2 تجزیه و پاسخ `25` با تأیید وریفایر برگردانده می‌شد (شاهد زمانی به‌دلیل نبودِ نشانهٔ `at/ساعت` پیش از ساعت، منبع را ناقص می‌دید). رفع: واقعبینی شاهد زمانی در `verifier_v22.py` — یک توکن ساعتِ لخت (HH:MM یا X o'clock) همزمان که نشانهٔ صریح ندارد، خودِ لحظهٔ شروع است. حالا پاسخ: `01:00 next day.`

### ۹-۴) بهبودهای پوشش (افزایشی)
- **نقش‌های عددی:** فعل ربطی («price is 200») نقش price را لنگر می‌کند؛ تخفیف یکایی («50 dollar discount»، «50 دلار تخفیف») تنها وقتی پذیرفته می‌شود که اسمِ تخفیف بلافاصله پس از واژهٔ یکا بیاید (ضدِ بازگشت باگ قیمت↔تخفیف ممیزی).
- **سرنخ‌های زمان‌بندی انگلیسی:** leaves/departs/opens + lasts/takes به شرط سه‌گانهٔ (سرنخ شروع، سرنخ پایان/مدت، یکای زمان) اضافه شد؛ «begins at» و مسیر NLU-retry نیز مینیت‌پرشن را دیکد می‌کند.
- **شش تست جدید** در `tests/test_v221_audit_fixes.py` (بخش ۷) این رفتارها را قفل می‌کند.

### ۹-۵) اعداد نهایی v22.2
| سنجه | مقدار |
|---|---|
| رگرسیون کامل | **1037 تست + 585 زیرتست — 0 خطا** |
| Fresh Blind صادقانه | **855/1023 = 83.58** (answerable-only 82.09) |
| Broad 500 | 406/500 = 81.2 (بدون تغییر؛ بدون ادعای زبانی) |
| Performance | v21 mean 5.196ms در برابر v22.2 mean 7.723ms (+48.6% mean، +72.0% p95) |
| زبان تولیدی | Persian 6/50، English 0/50 (بدون تغییر — صادقانه) |

### ۹-۶) شکاف‌های باز (صادقانه)
- شکاف‌های پوشش مدلِ فریم: عبارت‌های ناشناخته پاسخ `None` می‌دهند (abstain امن، هرگز عدد غلط) — نمونه: «A train departs at 23:30. The trip takes 45 minutes.»
- نرخ‌های ownership (19/60) و inventory (67/100) نسبت به v21 بدون تغییر — نیازمند ارتقای واقعی مدل.
- ارتقای واقعی زبان (مدل وزن‌دار) خارج از محدودهٔ این انتشار است.

**برچسب نهایی انتشار: v22.2 — Semantic/Verifier Audit-Response Final (Production Candidate).**
