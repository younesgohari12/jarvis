...
def enabled_lab_supervised_rows() -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    """
    state_path = ROOT / "datasets" / "catalog_v007.json"
    try:
    ...
    user = ROOT / "datasets" / "raw" / "user_entries_v007.jsonl"
    if user.is_file():
...
def tokenize() -> dict[str, Any]:
    ...
    report = {
        "format": "jarvis-v15-tokenization-report-v1",
        "dataset_version": VERSION,
        ...
        **lab_report,
        ...
        "note": "v15 uses dataset_v007 and merges enabled LAB supervised rows without duplicating built-in corpus files.",
        "pretrained_source": None,
    }
    metrics = ROOT / "models" / "tokenizer_metrics_v007.json"
